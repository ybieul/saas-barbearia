 ...
 */
function runSendTrialReminders() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const trialTenants = yield prisma_1.prisma.tenant.findMany({
                where: {
            ...
            for (const tenant of trialTenants) {
                ...
                if (daysLeft <= 0 && diffMs > 0) {
                    ...
                    try {
                        yield (0, email_1.sendTrialReminderEmail)(tenant.name, tenant.email, 0);
                        lastDayEmails++;
                        remindersSent++;
                    }
                    catch (error) {
                ...
                // Dia 13 (faltam 2 dias): quando restam exatamente 2 dias
                    ...
                    try {
                        yield (0, email_1.sendTrialReminderEmail)(tenant.name, tenant.email, 2);
                        twoDaysEmails++;
                        remindersSent++;
                    }
                    catch (error) {
            ...
            const expiredTenants = yield prisma_1.prisma.tenant.findMany({
                where: {
            ...
            for (const tenant of expiredTenants) {
                ...
                try {
                    yield (0, email_1.sendTrialExpiredMissYouEmail)(tenant.name, tenant.email);
                    missYouEmails++;
                    remindersSent++;
                }
                catch (error) {
