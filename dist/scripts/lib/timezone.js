 ...
 */
export function createBrazilDate(dateStr, timeStr) {
    try {
        let baseDate;
 ...
 */
export function formatBrazilTime(date, pattern = 'dd/MM/yyyy HH:mm') {
    try {
 ...
 */
export function getBrazilDayOfWeek(date) {
    const days = [
 ...
 */
export function getBrazilDayNameEn(date) {
    const days = [
 ...
 */
export function getBrazilDayNumber(date) {
    try {
        if (!date || !isValid(date)) {
 ...
 */
export function debugTimezone(date, context = 'Debug') {
    if (!date || !isValid(date)) {
        if (process.env.NODE_ENV === 'development') {
 ...
 */
export function addTimeToBrazilDate(brazilDate, hours, minutes = 0) {
 ...
 */
export function toBrazilDateString(date) {
    try {
        if (!date || !isValid(date)) {
 ...
 */
export function parseDatabaseDateTime(dateTimeString) {
    if (!dateTimeString) {
 ...
 */
export function getBrazilEndOfDay(date) {
 ...
 */
export function generateTimeSlots(startTime = '08:00', endTime = '18:00', intervalMinutes = 30) {
    const slots = [];
    try {
 ...
 */
export function utcToBrazil(date) {
 ...
 * @returns A mesma data (sem conversão)
 */
export function brazilToUtc(date) {
    // ⚠️ MIGRAÇÃO: Esta função agora é um pass-through
 ...
 */
export function extractTimeFromDateObject(date) {
    if (!date || !isValid(date)) {
