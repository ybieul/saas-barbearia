...
export const manualContent = [
   ...
   "Esta seção resume o resultado do NEGÓCIO no intervalo selecionado.",
   "",
   "- **Custos**: considera os custos mensais de forma INTEGRAL por mês dentro do período (sem prorrateio por dia).",
   "- **Comparação com período anterior**: compara com uma janela imediatamente anterior de mesma duração.",
   "  - Se anterior=0 e atual=0 → mostra '—' (neutro).",
   "  - Se anterior=0 e atual>0 → mostra 'Novo'.",
   "  - Caso contrário → variação em % (positiva/negativa).",
   "- **Colaboradores**: na visão de colaborador, os valores de 'receita/ganhos' e ticket consideram COMISSÕES; para Donos, considera o faturamento bruto.",
   "- **Posicionamento**: aparece logo abaixo da seção 'Custos Mensais'.",
   "",
   "Cards típicos:",
   "- Receita (ou Ganhos, para colaborador) no período",
   "- Comissões (totais) no período",
   "- Custos Fixos (integrais do período)",
   "- Lucro Líquido Estimado no período",
   "",
   "---",
   "",
   ...
   "- Atendimentos **Pré‑pago** entram na receita do período normalmente e aparecem na distribuição por métodos.",
   "- Comissões são calculadas e registradas ao concluir (snapshot), inclusive para **Pré‑pago**.",
   "- A 'Análise de Lucratividade do Período' já considera esses atendimentos e os custos integrais por mês.",
   "",
   "#### 3.8.5 Boas Práticas",
